...
class Maze(object):
  ...
  def __init__(self, width=None, height=None):
    ...
    self.mapping = {u'[0, 0]': u'  ', # Empty
                    u'[0, 1]': u' .', # Floor
                    u'[0, 2]': u' #', # Wall
                    u'[0, 3]': u' +', # Locked Door
                    u'[0, 4]': u' \'', # Unlocked Door
                    u'[0, 5]': u' C', # Chest
                    u'[0, 6]': u' S', # Secret
                    u'[0, 7]': u' T', # Trap
                    u'[0, 8]': u' M', # Mob
                    u'[0, 9]': u' >', # Up Stairs
                    u'[0, 10]': u' >', # Down Stairs
                    u'[0, 20]': u' $', # Loot: Gold
                    u'[0, 21]': u' P', # Loot: Potion
                    u'[0, 22]': u' &', # Loot: Gear
                    u'[0, 23]': u' |', # Loot: Weapon
                    u'[0, 24]': u' -', # Loot: Scroll
                    u'[0, 25]': u' ~', # Loot: Key
                    u'[0, 26]': u' %', # Loot: Bones
                    }
    self.previous_cells = []
